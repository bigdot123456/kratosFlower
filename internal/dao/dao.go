...
func newDao(r *redis.Redis, mc *memcache.Memcache, db *sql.DB) (d *dao, cf func(), err error) {
	var cfg struct{
		SvrflowerSvrExpire xtime.Duration
	}
	...
	d = &dao{
		...
		SvrflowerSvrExpire: int32(time.Duration(cfg.SvrflowerSvrExpire) / time.Second),
	}
	cf = d.Close
	return
}
